...
Rensic turns Alchemy transfer JSON into a Foundry case file a person can read.
...
```mermaid
flowchart LR
  Desk[OSDK React desk] -->|create / expand| Actions[Action types]
  ...
  Objects --> Desk
  Desk -->|label / flag| Fns
  Desk --> Summary[Summary from transfers + pack names]
```

Happy path in words:

1. Investigator opens a **file** (title, seed wallet, chain, lookback days).
2. A function writes an **Investigation Case** and a hop-0 **Case Address** for the seed.
3. The next pipeline build snapshots open cases into **ingestion requests**.
...
5. Those datasets back ontology objects the desk reads over OSDK.
6. The investigator ranks fund flow, opens ledgers, flags a note, or writes a **Summary** grounded in the same facts.

## Three packages, plus Foundry-native pieces

| Package | Role | How it reaches production |
|---|---|---|
| `rensic-app` | Vite + React OSDK desk | Commit, push, **git tag** for the hosted site |
...
| Investigator language | Object | Why |
|---|---|---|
| “this file / case” | Investigation Case | Seed, chains, window, status |
...
**Address** is the wallet whether or not a case exists. **Case Address** is “this wallet belongs in this case at hop N.” That split keeps global labels and case-scoped flow from collapsing into one kitchen-sink object.

Links are mostly object-backed through Case Address and case-scoped transfer objects, not a giant freeform graph dump.

## Desk surface

The UI noun is **file**, not “case,” in the chrome people see.

Tabs that matter for the demo:

- **Fund flow**: ETH between the file wallet and each counterparty
- **Addresses**: working set: seed pinned, labeled rows first, then by flow (capped so Vitalik-scale hop-1 does not become the product)
...
Ingest is **hop 1 only**: the seed and addresses it traded with in that window. That is the working set the desk ranks.

Alchemy has a transfer page cap (~200k). Busy public wallets can truncate. The desk should say when that happened. Truncation is a stress test, not a complete history.
...
- A name on screen should carry a source, be an investigator label, or stay unlabeled
- Unlabeled is valid. Missing label is not clearance
- Community names are not “Coinbase official”
- Official pack names are not freely renamed from the desk

Spark joins what it can onto addresses. The desk also reads pack metadata for categories and source URLs when showing Summary and chips.
...
- Expand window or refresh ingest request path
- Label / flag an address (investigator note)
- Delete investigation (cascade cleanup for case-scoped objects)
- Save narrative text when Summary is written to the file

Pipelines compute. Humans decide. Functions back the actions the desk calls through OSDK.

## Summary (grounded read)

Summary builds a closed brief from the file’s transfers and pack / investigator names, then turns that into short readable lines (intro, then biggest by ETH moved, one line each). It must not invent counterparties, hops, or scores.

Calling a Foundry AIP Agent from the desk was explored. On this Dev enrollment the reliable demo path is the local grounded paraphrase. The tab copy stays quiet about agents and RIDs.

## How a change ships

1. **Desk UI**: change `rensic-app`, push Stemma, tag (for example `0.1.22`). Hosted site follows the tag.
...
- Classic Foundry ontology + OSDK desk + TS functions + Spark
...
- Older classroom notes (archived): [archive/](archive/)
